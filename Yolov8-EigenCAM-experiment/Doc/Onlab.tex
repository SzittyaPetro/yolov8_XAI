...
A Pooling  ( \aref{fig:Pooling}. képen látható) réteget a bemenet térbeli dimenzióinak (szélességének és magasságának)
...
A Yolo (You only look once) \index{Yolo!Yolov5} algoritmus egy Joseph Redmon, Santosh Divvala, Ross Girshick és Ali Farhadi által kitalált
algoritmuson alapuló neurális háló amiből a félév során a Yolov5-öt (arhitektúra \aref{fig:architecture}. ábrán) használtam
...
Ezt a hálót tanítottam felül hogy az általam kitűzött cél elérésére, a közlekedési objektumok detektálására. A tanításához használtam fel a \("\)Cityscapes dataset\("\)\cite{cityscapes}-et\index{CityScapes}.
...
\end{enumerate}
    ...
    A YOLOv8 (\gls{Yolov8}) \index{Yolo!Yolov8}egy népszerű és hatékony konvolúciós neurális háló.
...
\newsection{YOLOv8: Szemantikus szegmentációs háló és működése}\label{sec:yolov8:-szemantikus-szegmentacios-halo}
    \begin{wrapfigure}{l}{0.2\textwidth}
    	\centering
    	\vspace{-10pt} % Optional: Adjust this value to fine-tune vertical spacing
...
A szemantikus szegmentáció\index{szegmentáció!szemantikus szegmentáció} egy olyan adatreprezentációs forma, amelyben a cél az
objektumokat tartalmazó kép egyes részeinek (például pixeljeinek) címkézése az objektumokhoz tartozó osztályok szerint.
Van emellett egy másik szegmentációs adatreprezentációs forma, az ún. egyed szegmentáció\index{szegmentáció!egyedszegmentáció}
...
\subsection{Adathalmazok és kísérletek}\label{subsec:adathalmazok-es-kiserletek}
    ...
    A kísérletek \aref{tab:Activations}. táblázatban találhatóak.
...
A Comet.ml (\url{https://www.comet.ml}) egy kollaboratív \ac{MLOps} platform, amely lehetővé teszi, hogy könnyen és
    hatékonyan figyelhessük a futó tanításainkat, validációinkat és \glsentryshort{Inference}inket.
...
\newsection{EigenCAM: Modellfüggőn magyarázó}\label{sec:eigencam:-modellfuggo-magyarazo}
    ...
    Ezt a mátrixot kiterjeszti (ha kell) annak a képnek méretére amire éppen az \glsentryshort{Inference}et futtatjuk.
...
\newsection{Az EigenCAM alkalmazása a YOLOv8-ra}\label{sec:az-eigencam-alkalmazasa-a-yolov8-ra}
    ...
    amit \glsentryshort{Inference}nek nevezünk).
...
\begin{itemize}\label{itemize}
        ...
        \item Teli nyíl: \glsentryshort{Inference} (EigenCAM használata) közben.\label{Inference}
        \item Laza szaggatott nyíl: \glsentryshort{Inference} idő után.
    \end{itemize}
...
\subsection{Eredmények összegzése}\label{subsec:osszegzes-es-kovetkeztetes}
    ...
    \ac{MLOps} megoldásokat munkám segítésére, mint például a Comet (\url{https://www.comet.ml}) amelyben az ott végzett munkámat és szerzett tapasztalataimat \aref{subsec:mlops}. fejezetben részleteztem.
...
\newsection{Jövőbeli irányok és kutatási lehetőségek}\label{sec:jovobeli-iranyok-es-kutatasi-lehetosegek}
    ...
    témakörében.
    Azon belül a szemantikus szegmentációban\index{szegmentáció!szemantikus szegmentáció}, ami az elmúlt években előnyt élvez
